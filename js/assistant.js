...
(function () {
  ...
  var DOMAIN = ['上市', '挂牌', '纳斯达克', '纽交所', '美股', '港股', 'a股', '新加坡', '香港',
    ...
    // 同义词补登（2026-09-16）：GA4 里读到真实访客因为少这几个词被当成闲聊回绝，
    // 例「尽职调查查什么」（表里只有「尽调」）、「条款里最该守住哪几条」、「漏了登记还能补吗」
    '尽职调查', '条款', '登记', '外汇', '股东', '董事', '监事', '控制权', '实际控制人',
    '标的', '壳', '轮次', '投资人', '净利', '营收', '利润', '报表', '主体', '协议',
    '章程', '限售', '解禁', '路径', '交割', '重组', '注入'];
  ...
  /* 本次会话里是否已经问过业务问题。
     领域词闸原来是无状态的：只看当前这一句有没有业务词 —— 而「追问」天然没有
     （上一句问架构，下一句问「条款里最该守住哪几条」），于是客户往下挖的那一刻被回绝。
     置位后后续问题一律放行走检索，答不上来也只是 miss/partial，不再当闲聊赶人。 */
  var domainCtx = false;

  /* 真人来搭话谈合作（自我介绍口吻），不该收到「不在本站范围内」。
     2026-09-16 从 GA4 里读到一条真实的：一家越南企业服务机构在助手里做自我介绍想谈合作，
     被领域词闸判成 offtopic 回绝掉了，没有任何人看到过。 */
  var BIZ_INTRO = ['我们是', '我司', '敝公司', '本公司是', '我叫', '自我介绍',
    '寻求合作', '想合作', '谈合作', '洽谈', '双方是否', '是否有合作', '合作机会',
    '服务机构', '服务商', '代理商', '渠道商', '介绍我们'];
  function isBizIntro(q) {
    var n = norm(q), i;
    for (i = 0; i < BIZ_INTRO.length; i++) { if (n.indexOf(norm(BIZ_INTRO[i])) > -1) return true; }
    return false;
  }
  ...
  function answer(q) {
    ...
    // 真人来谈合作：给联系方式，⛔ 不要回一句「不在范围内」把人赶走
    if (isBizIntro(q)) {
      track('bizintro', q.slice(0, 80));
      addBot('谢谢你的介绍。合作类的事情这边由团队直接对接 —— 邮箱 ' + EMAIL + '，微信 ' + WECHAT + '。<br>' +
        '邮件里写清你们机构做什么、在哪个地区、希望以什么方式合作，会更快得到回复。' +
        ctaHTML('合作洽谈请直接联系团队。'));
      return;
    }

    // 领域词准入：问题里没有任何业务词就不硬凑答案
    // ⚠ 但本次会话里已经聊过业务的，后面的追问一律放行（追问天然不带业务词）
    if (!hasDomain(q)) {
      if (!domainCtx) {
        track('offtopic', q.slice(0, 40));
        addBot('这个问题不在本站内容范围内。我只回答境外上市与跨境资本运作相关的问题，' +
          '例如上市路径、纳斯达克门槛、费用与周期、VIE架构、37号文与ODI备案、并购重组等。' +
          ctaHTML('其他事宜可直接联系团队。'));
        return;
      }
      track('followup', q.slice(0, 40));
    } else {
      domainCtx = true;
    }
